...
res.pair <- dm_pair(
   ...
   marker1 = "mut_ARID1A", m1.cat.pos = "YES", m1.cat.neg = "NO",
   #marker2 = "gepscore_CD.8.T.effector",
   #marker2 = "gep_MUC1"
   marker2 = "mut_NCOR1", m2.cat.pos = "YES", m2.cat.neg = "NO"
   )
purrr::walk(res.pair$response.plot, print)
purrr::walk(res.pair$survival.plot, print)
```
...
## Example: response ~ mut_ARID1A + gepscore
```{r}
...
res.dm.logit.gepscore <- dm_searchM2_logit(
   ...
   response.neg = "SD/PD", 
    # marker1
   marker1 = "mut_ARID1A", m1.binarize = T, m1.cat.pos = "YES", m1.cat.neg = "NO",
    # marker2
...
## Example: OS ~ mut_ARID1A + gepscore
```{r}
m2.candidates <- str_subset(colnames(clin_bmk_IMvigor210),"gepscore_")
length(m2.candidates)
...
## Example: Response ~ mut_ARID1A + gep
...
## Example: survival ~ mut_ARID1A + gep
Here we demonstrated searchM2 function to combine with TMB for signatures
```{r}
m2.candidates <- str_subset(colnames(clin_bmk_IMvigor210),"gep_")
length(m2.candidates)
...
sm_searchM_topPlot(res.sm.logit.mut, top.n = 20)
```

## Example: response ~ mut_ARID1A + mutation
```{r}
...
## Example: survival ~ mut_ARID1A + mutation
```{r}
m2.candidates <- str_subset(colnames(clin_bmk_IMvigor210),"mut_")
length(m2.candidates)
res.dm.logit <- dm_searchM2_cox(
   data = clin_bmk_IMvigor210, 
    # response info
   time =  "os", event = "censOS",
   # marker1
