...
clin.info.gep <- cds %>% pData() %>%
  ...
  rownames_to_column("sample_id_gep") %>%
  as_tibble()
...
names(human_gene_signatures) %<>%
  make.names() %>%
  str_replace(., pattern = "gene19",replacement = "TGFb.19gene")
# add hallmark
genesets <- fun.read.gmt("data-raw/h.all.v7.0.symbols.gmt")
human_gene_signatures <- c(human_gene_signatures, genesets)
# pre-process
names(human_gene_signatures) %<>% make.names()
human_gene_signatures %<>% purrr::keep(.p = ~{length(intersect(.x, rownames(es.gep)))>0})


sig.score <- purrr::map_dfr(human_gene_signatures, .f = ~{
  goi <- intersect(.x, rownames(es.gep))
  IMvigor210CoreBiologies::gsScore(gm = exprs(es.gep[goi,])) %>% t %>% as_tibble()
}, .id ="sig_name")
...
genesets <- fun.read.gmt("data-raw/panel_gene_list.txt")
goi <- c(genesets$HTG_PIPIO, "FCGR1A", "FCGR1B")
#goi <- rownames(es.gep)
...
# save
save(clin.bmk, clin.info.gep, clin.info.mut, es.gep, es.gepscore, mut.wide,
     file = "data-raw/my_IMvigor210_data.RData")
library(devtools)
clin_bmk_IMvigor210 <- clin.bmk
use_data(clin_bmk_IMvigor210)
